...
void test_rsp_file(string rsp_file_dir) {
  ...
  string line;
  unsigned char sha_hash[SHA1_HASH_SIZE];
  ...
  while (getline(infile, line)) {
    if (line.length() >= 3 && line.substr(0, 3) == "Len") {
      ...
      cal_sha1_hash(msg_bytes, next_msg_len / CHAR_BIT, sha_hash);
      cout << "MD = " << bytes_to_hex_string(sha_hash, SHA1_HASH_SIZE) << " ";
    } else if (line.length() >= 2 && line.substr(0, 2) == "MD") {
      string official_md = line.substr(5, line.length() - 5 - 1);
      if (bytes_to_hex_string(sha_hash, SHA1_HASH_SIZE) == official_md) {
        cout << "Passed" << endl;
      } else {
...
int main() {
    test_rsp_file("../../test-vectors/SHA1ShortMsg.rsp");
    test_rsp_file("../../test-vectors/SHA1LongMsg.rsp");
    return 0;
}