...
int test_base32_encode() {
  ...
  for (int i = 0; i < TEST_COUNT; ++i) {
    output =
        encode_bytes_to_base32_string((unsigned char *)official_tvs_decoded[i],
...
int test_base32_decode_valid() {
  ...
  for (int i = 0; i < TEST_COUNT; ++i) {
    output =
        decode_base32_string_to_bytes(official_tvs_b32_encoded[i], &output_len);
...
int test_base64_encode_official() {
  ...
  for (int i = 0; i < TEST_COUNT; ++i) {
    output =
        encode_bytes_to_base64_string((unsigned char *)official_tvs_decoded[i],
                                      strlen(official_tvs_decoded[i]), 0);
...
int test_base64_encode_official_wrapped() {
  const char official_tvs_b64_encoded_wrapped[][TEST_SIZE] = {
      "", "Zg==", "Zm8=", "Zm9v\n", "Zm9v\nYg==", "Zm9v\nYmE=", "Zm9v\nYmFy\n"};
  char *output;
  for (size_t i = 0; i < sizeof(official_tvs_b64_encoded_wrapped) /
                             sizeof(official_tvs_b64_encoded_wrapped[0]);
       ++i) {
    output =
        encode_bytes_to_base64_string((unsigned char *)official_tvs_decoded[i],
                                      strlen(official_tvs_decoded[i]), 4);
...
int test_base64_decode_official() {
  ...
  for (int i = 0; i < TEST_COUNT; ++i) {
    output =
        decode_base64_string_to_bytes(official_tvs_b64_encoded[i], &output_len);
...
int test_base64_google() {
  ...
  int64_t output_len = -1;
  // From Google:
  // https://boringssl.googlesource.com/boringssl/+/master/crypto/base64/base64_test.cc
  ...
  char goog_decoded[][128] = {
      ...
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
      "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"};
  for (size_t i = 0; i < sizeof(goog_tv_encoded) / sizeof(goog_tv_encoded[0]);
       ++i) {
  ...
  char goog_tv_encoded_invalid[][128] = {
      ...
      "ZW5jb2RlIG1lCgCC====\n"};
  printf(
      "===== Testing invalid input, some error messages are expected =====\n");
  for (size_t i = 0;
       i < sizeof(goog_tv_encoded_invalid) / sizeof(goog_tv_encoded_invalid[0]);
       ++i) {
    output =
        decode_base64_string_to_bytes(goog_tv_encoded_invalid[i], &output_len);
