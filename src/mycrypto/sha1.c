/**
 * A very straightforward and no-frills C implementaion of SHA1 based on the
 * pseudocode at https://en.wikipedia.org/wiki/SHA-1
 */
...
int cal_sha1_hash(const unsigned char *input_bytes, const size_t input_len,
   ...
   * character). begin with the original message of length L bits append the bit
   * '1' to the message e.g. by adding 0x80 if message length is a multiple of 8
   * bits. append 0 ≤ k < 512 bits '0', such that the resulting message length
   * in bits is congruent to −64 ≡ 448 (mod 512) append ml, the original message
   * length in bits, as a 64-bit big-endian integer. Thus, the total length is a
   * multiple of 512 bits.
   *
   * The padding scheme is the same as that of SHA256.
   */
  size_t padded_len = input_len + (SHA1_CHUNK_SIZE_BIT -
                                   input_len * CHAR_BIT % SHA1_CHUNK_SIZE_BIT) /
                                      CHAR_BIT;
