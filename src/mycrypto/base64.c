...
char *encode_bytes_to_base64_string(const unsigned char *input_bytes,
  ...
  while (input_end - input_start >= 3) {
    // Let's have an example here, say we need to encode three 3 bytes 01001111
    // 11100110 10010111
    *pos++ = base64_table[input_start[0] >> 2];
    // The funny (*pos++ = val) means we assign val to *pos and then we point
    // pos to the next address (i.e., pos + 1) encode the first 6 bits,
    // [010011]11 by right shifting them: 00[010011]
    *pos++ =
        base64_table[((input_start[0] & 0x03) << 4) | (input_start[1] >> 4)];
    // encode the 7th to 12th bits, 010011[11 1110]0110
    // extract the last two bits: input_start[0] & 0x03: 01001111 & 0000011 =
    // 00000011 shift them to lower places:  ((input_start[0] & 0x03) << 4):
    // 00110000 extract the first 4 bits from the 2nd byte: input_start[1] >> 4:
    // 11100110 >> 4 = 00001110 concatenate last 2 bits from 1st byte and first
    // 4 bits from 2nd byte: 00110000 | 00001110 = 00111110
    *pos++ =
        base64_table[((input_start[1] & 0x0f) << 2) | (input_start[2] >> 6)];
  ...
  if (input_end - input_start) {
    // There could be one or two remaining bytes, either case, we need to encode
    // the first 6 bits and ensure the final result is 3-byte (i.e., 24-bit)
    // long.
    *pos++ = base64_table[input_start[0] >> 2];
    if (input_end - input_start == 1) {
      // If there is one byte left, say, 11100111, we want to make it 11100111
      // 00000000 00000000 we need to encode it into two characters and two
      // extra padding character as follows [111001][11 0000][0000 00][000000]
      *pos++ = base64_table[(input_start[0] & 0x03) << 4];
      // [111001] is done before if, now we handle the 111001[11 0000]0000 part
      *pos++ = '=';
      // handles 11100111 0000[0000 00]000000 part. The last 6 bits will be
      // handled after if.
    } else {
      // If there are two bytes left, say, 11100111, we want to make it 11100111
      // 01001001 00000000
      *pos++ =
          base64_table[((input_start[0] & 0x03) << 4) | (input_start[1] >> 4)];
...
unsigned char *decode_base64_string_to_bytes(const char *input_chars,
  ...
  if (valid_char_cnt % 4) {
    fprintf(stderr,
            "Unexpected input_chars length: not a multiple of 4 (%ld)\n",
            valid_char_cnt);
    *output_len = -1;
    return NULL;
  }
