...
void cal_sha256_hash(const uint8_t* input_bytes, const size_t input_len, uint8_t* hash) {
     ...
     * 
     */
    size_t padded_len = input_len + (SHA256_CHUNK_SIZE_BIT - input_len * CHAR_BIT % SHA256_CHUNK_SIZE_BIT) / CHAR_BIT;
    ...
    for (int i = 0; i < chunk_count; ++i) {
        ...
        for (int j = 16; j < 64; j++) {
            const uint32_t s0 = rotate(w[j-15], 7, false) ^ rotate(w[j-15], 18, false) ^ (w[j-15] >> 3);            
			const uint32_t s1 = rotate(w[j-2], 17, false) ^ rotate(w[j-2],  19, false) ^ (w[j-2] >> 10);
        ...
        for (int j = 0; j < 64; ++j) {
            const uint32_t S1 = rotate(e, 6, false) ^ rotate(e, 11, false) ^ rotate(e, 25, false);
            const uint32_t ch = (e & f) ^ ((~e) & g);
            const uint32_t temp1 = h + S1 + ch + k[j] + w[j];
            const uint32_t S0 = rotate(a, 2, false) ^ rotate(a, 13, false) ^ rotate(a, 22, false);
